...
#include <math.h> // 引入数学库以使用round函数  
  #include "NTP_Task.h"
/******************************* 全局变量声明 ************************************/
/*
...
uint8_t MQTT_Connect(void)
{
    ...
    //发送消息
    transport_sendPacketBuffer(buf, len);//发送connect

    /* 等待连接响应 */
    if (MQTTPacket_read(buf, buflen, transport_getdata) == CONNACK)//返回connack
    {
...
//		printf("mqtt北京时间：%02d-%02d-%02d %02d:%02d:%02d\r\n",  
//                           g_nowdate.year, 
//                           g_nowdate.month + 1,
//                           g_nowdate.day,
//                           g_nowdate.hour + 8,
...
MQTT_SEND_START:
    ...
    xReturn = xQueueReceive( MQTT_DHT11_Data_Queue,    /* 消息队列的句柄 */
                             &recv_data,      /* 发送的消息内容 */
                             3000); /* 等待时间 3000ms */
      if(xReturn == pdTRUE)
      {
